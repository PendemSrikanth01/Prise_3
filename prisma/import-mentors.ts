...
async function main() {
  ...
  const existing = await prisma.person.findMany({ where: { role: Role.MENTOR }, select: { id: true, name: true, email: true, isActive: true, _count: { select: { assignments: true, facilitatedSessions: true, activityLogs: true } } }, orderBy: { name: 'asc' } });
