...
export default async function AssignMilestonesPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [startup, templates, assigned] = await Promise.all([
    ...
    prisma.milestone.findMany({ where: { startupId: id, templateId: { not: null }, status: { not: 'NA' } }, select: { templateId: true } }),
  ]);
  if (!startup) notFound();
  ...
  return <div className="mx-auto w-full max-w-5xl p-4 sm:p-6 lg:p-8"><Link href={`/startups/${id}`} className="text-sm font-semibold text-prise-primary">← {startup.name}</Link><div className="mt-5"><div className="rounded-card bg-prise-sidebar p-6 text-white"><p className="text-sm text-white/55">Joint founder + mentor planning</p><h1 className="mt-2 text-2xl font-bold">Build a milestone plan around this startup</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-white/65">Select only the milestones this startup needs. You can unselect previous milestones and propose replacements; existing evidence remains safely archived.</p></div><AssignMilestonesForm startupId={id} templates={templates} selectedIds={selectedIds} program={program} /></div></div>;
}
