...
export default async function AuditPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const [events, startups] = await Promise.all([
    prisma.activityLog.findMany({ where: { startupId: startupId || undefined, entityType: entityType || undefined }, orderBy: { createdAt: 'desc' }, take: 200, include: { actor: { select: { name: true } }, startup: { select: { name: true } } } }),
