...
export default async function WorkPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const [tasks, startups, programPeople, openCount, blockedCount, overdueCount] = await Promise.all([
    ...
    prisma.startup.findMany({ where: startupScope, orderBy: { name: 'asc' }, select: { id: true, name: true, milestones: { orderBy: [{ phase: 'asc' }, { title: 'asc' }], select: { id: true, title: true } }, memberships: { where: { isActive: true }, select: { personId: true, role: true, person: { select: { id: true, name: true, role: true } } } }, assignments: { select: { person: { select: { id: true, name: true, role: true } } } }, founder: { select: { id: true, name: true, role: true } } } }),
  ...
  return <div className="mx-auto w-full max-w-[1400px] p-4 sm:p-6 lg:p-8">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><div className="text-xs font-semibold uppercase tracking-[.12em] text-prise-primary">Execution workspace</div><h1 className="mt-1 text-2xl font-bold tracking-tight">Work</h1><p className="mt-1.5 text-sm text-prise-text-secondary">Create, assign, discuss and finish the work that moves each startup forward.</p></div><Link href="/tickets" className="inline-flex h-11 items-center justify-center gap-2 rounded-button border bg-white px-4 text-sm font-semibold text-prise-primary"><MessageCircle size={17} />Need help? Raise ticket</Link></div>
    ...
    <details className="mt-5 rounded-card border bg-white p-5 shadow-card"><summary className="flex cursor-pointer list-none items-center gap-2 font-semibold text-prise-primary"><Plus size={17} />Create task</summary><div className="mt-5"><TaskCreateForm startups={createOptions} currentUserId={session.user.id} /></div></details>
