...
export default async function NotificationsPage({ searchParams }: { searchParams: Promise<{ tab?: string }> }) {
  ...
  return <div className="mx-auto w-full max-w-[1350px] p-4 sm:p-6 lg:p-8">
    ...
    <p className="mb-4 text-sm text-prise-text-secondary">From: {from}. Replies go to {replyTo}. Sent means accepted by the email provider, not confirmed inbox delivery.</p>
    {!process.env.AUTOMATION_SECRET ? <p role="status" className="mb-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">Scheduled emails and reminders need server setup. Send-now remains available when delivery is enabled. The program lead must configure the automation secret and start the notification worker.</p> : null}
