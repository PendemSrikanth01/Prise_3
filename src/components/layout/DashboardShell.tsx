...
export function DashboardShell({ children, user, canViewAudit, canManagePeople, notifications, unreadNotificationCount }: { children: React.ReactNode; user: ShellUser; canViewAudit: boolean; canManagePeople: boolean; notifications: ShellNotification[]; unreadNotificationCount: number }) {
  ...
  return (
    <ToastProvider><div className="min-h-screen md:flex">
      ...
      <div className="min-w-0 flex-1 pb-16 md:pb-0">
        <TopBar onMenu={() => setMobileOpen(true)} mobileOpen={mobileOpen} userName={user.name} notifications={notifications} unreadCount={unreadNotificationCount} />
