...
export async function FounderDashboard({ user }: { user: FounderUser }) {
  ...
  return <div className="mx-auto w-full max-w-[1500px] p-4 sm:p-6 lg:p-8">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><h1 className="text-2xl font-bold tracking-[-.025em] sm:text-[28px]">Good morning, {user.name.split(' ')[0]}</h1><p className="mt-1.5 text-sm text-prise-text-secondary">Keep {startup.name} moving—one clear next action at a time.</p></div>{nextMilestone ? <Link href="/documents" className="inline-flex h-11 items-center justify-center gap-2 rounded-button bg-prise-sidebar px-5 text-sm font-semibold text-white"><Target size={17} />Continue next milestone</Link> : null}</div>
