  ...
  return (
    <div className="mx-auto w-full max-w-[1500px] p-4 sm:p-6 lg:p-8">
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.7fr)_minmax(320px,.8fr)]">
        ...
        <div className="space-y-5">
          <Panel title="Support requests" href="/support">
            {supportRequests.length ? supportRequests.map((request) => <Link href="/support" key={request.id} className="block border-t px-5 py-4 first:border-t-0 hover:bg-prise-page/70"><div className="text-sm font-semibold">{request.title}</div><div className="mt-1 flex justify-between gap-3 text-xs text-prise-text-secondary"><span>{request.startup.name}</span><span>{request.status.replaceAll('_', ' ').toLowerCase()}</span></div></Link>) : <Empty text="No open support requests." compact />}
          </Panel>
