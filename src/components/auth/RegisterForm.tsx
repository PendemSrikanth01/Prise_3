...
const inputClass = 'h-11 w-full rounded-input border border-prise-border bg-white px-4 text-sm text-prise-text outline-none transition placeholder:text-prise-text-muted focus:border-prise-primary focus:ring-4 focus:ring-prise-primary/10';
...
export function RegisterForm() {
  ...
  return (
    <form action={action} className="space-y-4">
      ...
      <button disabled={pending} className="group flex h-12 w-full items-center justify-center gap-4 rounded-button bg-prise-action px-5 text-sm font-bold text-white shadow-[0_10px_24px_rgb(229_57_61/20%)] transition hover:bg-prise-action-hover disabled:opacity-60">
...
function Field({ id, name, label, type = 'text', autoComplete, placeholder, minLength, maxLength }: { id: string; name: string; label: string; type?: string; autoComplete: string; placeholder: string; minLength?: number; maxLength?: number }) {
  return <div><label htmlFor={id} className="mb-2 block text-sm font-semibold text-prise-text">{label}</label><input id={id} name={name} type={type} autoComplete={autoComplete} required minLength={minLength} maxLength={maxLength} className={inputClass} placeholder={placeholder} /></div>;
}
