...
function EvidenceList({ milestone, canReview }: { milestone: MilestoneItem; canReview: boolean }) {
  return <div>{milestone.deliverables.length ? <div className="space-y-2">{milestone.deliverables.map((file) => <div key={file.id} className="rounded-lg border bg-white p-3"><div className="flex flex-wrap items-center justify-between gap-3"><div className="min-w-0"><a href={`/api/documents/${file.id}`} className="inline-flex max-w-full items-center gap-2 text-sm font-semibold text-prise-primary"><Download size={14} /><span className="truncate">{file.name}</span></a><div className="mt-1 text-[11px] text-prise-text-secondary">v{file.version} · {file.uploader?.name ?? 'Team member'} · {file.sizeBytes ? `${(file.sizeBytes / 1024 / 1024).toFixed(1)} MB` : 'file'}</div></div><Badge value={file.status} /></div>{file.description ? <p className="mt-2 text-xs text-prise-text-secondary">{file.description}</p> : null}{file.feedback ? <p className="mt-2 rounded-lg bg-prise-page p-2 text-xs text-prise-text-secondary">Review: {file.feedback}</p> : null}{canReview && file.status === DeliverableStatus.SUBMITTED ? <form action={reviewDeliverableAction} className="mt-3 grid gap-2 sm:grid-cols-[150px_1fr_auto]"><input type="hidden" name="deliverableId" value={file.id} /><select name="status" defaultValue={DeliverableStatus.APPROVED} className={inputClass}><option value={DeliverableStatus.APPROVED}>APPROVE</option><option value={DeliverableStatus.NEEDS_REVISION}>NEEDS REVISION</option></select><input name="feedback" placeholder="Review feedback" className={inputClass} /><SubmitButton className="!py-2">Review</SubmitButton></form> : null}</div>)}</div> : <p className="rounded-lg border border-dashed p-3 text-xs text-prise-text-secondary">No milestone files yet.</p>}</div>;
}
