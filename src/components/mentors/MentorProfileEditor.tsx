...
export function MentorProfileEditor({ mentor, canEdit }: { mentor: MentorProfileData; canEdit: boolean }) {
  return <div className="mx-auto w-full max-w-5xl p-4 sm:p-6 lg:p-8">
    <div className="rounded-card border bg-white p-5 shadow-card sm:p-7">
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-3"><ProfileFact icon={BriefcaseBusiness} label="Assigned startups" value={`${mentor._count.assignments}`} /><ProfileFact icon={Award} label="Experience" value={mentor.yearsExperience === null ? 'Not added' : `${mentor.yearsExperience} years`} /><ProfileFact icon={Languages} label="Languages" value={mentor.languages.join(', ') || 'Not added'} /></div>
    </div>
